          ...
          <div class="col-md-12">
            <div class="card">
              ...
              <?php 
                ...
                $sql = mysqli_query($conn, "SELECT * FROM patient WHERE (DATE(date_registered) 
                  BETWEEN '$startDate' AND '$endDate') ORDER BY date_registered DESC ");
              ...
              <?php if(mysqli_num_rows($sql) > 0) { ?>
                ...
                <div id="printElement">
                  ...
                  <hr>
                  <p class="text-center"><b>PATIENT RECORDS BETWEEN <?= strtoupper(date("F d, Y", strtotime($weeklyStartDate))).' - '.strtoupper(date("F d, Y", strtotime($weeklyEndDate))); ?></b></p>
              ...
              <?php } } elseif(isset($_POST['monthly'])) { 
                ...
                $currentYear = date('Y');
                $sql = mysqli_query($conn, "SELECT * FROM patient WHERE MONTH(date_registered) = '$monthlyMonth' ORDER BY date_registered DESC  ");
              ?>
              <?php if(mysqli_num_rows($sql) > 0) { ?>
                <button id="printButton" class="btn btn-success btn-sm float-sm-right mr-3"><i class="fa-solid fa-print"></i> Print</button>
                <div id="printElement">
                  ...
                  <hr>
                  <p class="text-center"><b>PATIENT RECORDS ON THE MONTH OF <?= strtoupper(date("F", strtotime("$currentYear-$monthlyMonth-01"))) . " $currentYear"; ?> </b></p>
                  <table id="" class="table table-bordered table-hover text-sm table-sm">
                    <thead>
                    <tr> 
                      <th>NAME</th>
