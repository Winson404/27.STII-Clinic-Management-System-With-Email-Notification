<title>STII Clinic Management System | Returned Medicine records</title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row">
          <div class="col-sm-6">
            <h3>Returned Medicine records</h3>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="dashboard.php">Home</a></li>
              <li class="breadcrumb-item active">Returned Medicine records</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="col-md-12">
            <div class="card">
              ...
              <?php 
                ...
                $sql = mysqli_query($conn, "SELECT * FROM medicine WHERE DATE(expiration_date)<CURDATE() AND is_returned=1 AND (DATE(expiration_date) BETWEEN '$startDate' AND '$endDate') ORDER BY expiration_date DESC ");
              ?>
              ...
              <?php if(mysqli_num_rows($sql) > 0) { ?>
                ...
                <div id="printElement">
                  ...
                  <hr>
                  <p class="text-center"><b>RETURNED MEDICINE RECORDS BETWEEN <?= strtoupper(date("F d, Y", strtotime($weeklyStartDate))).' - '.strtoupper(date("F d, Y", strtotime($weeklyEndDate))); ?></b></p>
              ...
              <?php if(mysqli_num_rows($sql) > 0) { ?>
                ...
                <div id="printElement">
                  ...
                  <hr>
                  <p class="text-center"><b>RETURNED MEDICINE RECORDS BETWEEN <?= strtoupper(date("F", strtotime($monthlyStartDate))).' - '.strtoupper(date("F", strtotime($monthlyEndDate))).' '.$currentYear ?> </b></p>
                  <table id="" class="table table-bordered table-hover table-sm text-sm">
                    <thead>
                      <tr> 
                        <th>BRAND NAME</th>
              ...
              <?php } } else { ?>
                 <table id="example111" class="table table-bordered table-hover text-sm">
                    ...
                    <tbody id="users_data">
                        <?php 
                          $sql = mysqli_query($conn, "SELECT * FROM medicine WHERE DATE(expiration_date)<CURDATE() AND is_returned=1 ");
                          while ($row = mysqli_fetch_array($sql)) {
                        ?>
                      <tr>
                          <td><?php if($row['brand_name'] == 'Others') { echo ucwords($row['other_brand_name']); } else { echo $row['brand_name']; }; ?></td>
