<?php 
	...
	if(isset($_POST['request_appointment'])) {
		...
		$selectedYear = date('Y', strtotime($appt_date));
		$currentYear = date('Y');
		// GET PATIENT NAME
		...
		if(mysqli_num_rows($get_limit) >= 8) {
			...
			if ($selectedYear == $currentYear) {
					...
					if(mysqli_num_rows($check) > 0) {
						...
						header("Location: appointment.php");
						  ...
						  if($save) {
						  		...
						  		$mess = 'Good day '.$gender.' '.$admin_name.', an appointment has been set by new patient named, '.$name.'.';
						  		...
						  		if($save2) {
								      ...
								      $message = '<p>Good day '.$gender.' '.$admin_name.', an appointment has been set by new patient named, '.$name.'.</p>
				...
				$_SESSION['message'] = "Year must be current Year";
		        $_SESSION['text'] = "Please try again.";
		        $_SESSION['status'] = "error";
				header("Location: appointment.php");
			}
	...
	if(isset($_POST['request_medical_certificate'])) {
		...
		$selectedYear = (new DateTime($pick_up_date))->format('Y');
		$currentYear = date('Y');
		if ($selectedYear == $currentYear) {
				...
				if(mysqli_num_rows($appt_exists) > 0 ) {
					...
					$save = mysqli_query($conn, "INSERT INTO request_doc (type, patient_Id, purpose, pick_up_date) VALUES ('$type', '$patient_Id', '$purpose', '$pick_up_date')");
					  if($save) {
					  		...
					  		$mess = 'Good day '.$gender.' '.$admin_name.', a request for medical records has been set by new patient named, '.$name.'.';
	...
	// DISPLAY THE APPT TIME(APPOINTMENT ADD) AUTOMATICALLY THAT ARE NOT IN THE APPOINTMENT TIME COLUNN FOR THE SELECTED DATE
